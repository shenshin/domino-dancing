...
class DominoController {
    constructor() {
        this.game = new DoninoInteractiveGame({ delegate: this, tilesNumber: 7 });
    ...
    gameOverWarning() {
        this.messageField.className = 'warning';
        const many = this.game.winners.length > 0;
        const winners = this.game.winners.map((w) => w.name).join(', ');
    ...
    onWin(winnersNames, tilesLeft = 0) {
        ...
        if (winnersNames.length === 1) {
            ...
            if (tilesLeft > 0) {
                ...
                this.dominoLog.innerText += `${name} still has ${tilesLeft} tile${tilesLeft > 1 ? 's' : ''} in the stock, but the other${manyPlayers ? 's have' : ' player has'} more.`;
            }
        }
